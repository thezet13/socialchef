import { FormatId } from "@/modules/ai/types/ai.types";
...
//   const core = `
// You have TWO images:
// 1) The user's dish photo (base image).
// 2) The style reference image.

// Your task is to apply the VISUAL STYLE of image #2 to image #1.

// Image #2 is used for STYLE ONLY.
// Do NOT copy composition, framing, or camera distance from image #2.

// IDENTITY (MUST KEEP):
// - Same type of food
// - Same ingredients and structure
// - Realistic proportions and textures
// - Do NOT change the dish shape or geometry

// STYLE TRANSFER (CRITICAL):
// - Treat the dish as if it was photographed in the style setup of image #2.
// - Rebuild lighting on the dish to match the style:
//   direction, hardness/softness, highlight placement, shadow shape and density.
// - Apply the style’s color grading to the entire image:
//   temperature, contrast, saturation.
// - The dish MUST look relit and recolored according to the style.
// - Do NOT preserve the input image’s original lighting if it conflicts with the style.

// Apply this visual style (STRICT):
// ${opts.styleText}

// COMPOSITION & FRAMING (ABSOLUTE RULES):
// - Keep the EXACT composition and framing of image #1.
// - The output MUST be pixel-aligned with image #1.
// - NO zoom-in, NO zoom-out.
// - NO crop.
// - NO reframe.
// - NO change of camera distance.
// - Preserve margins to the image edges EXACTLY as in image #1.
// - Do NOT tighten or loosen the crop.
// - Do NOT create a “hero” close-up.

// FOOD CONTAINER / SERVING SURFACE (STRICT):
// - The dish may be placed on a serving surface or container (e.g. plate, board, paper, box, tray, bowl).
// - Treat the serving surface as a SURFACE MATERIAL, not as a geometric object.
// - You MAY change the color, material, texture, or finish of the serving surface to match the style.
// - KEEP the exact outline, size, perspective, and visible boundaries of the serving surface.
// - Do NOT change the shape, geometry, or position of the serving surface.
// - The serving surface MUST remain pixel-aligned with the input image.
// - The original serving surface appearance should NOT be recognizable if it conflicts with the style.


// BACKGROUND:
// - Match the background mood and atmosphere of the style reference.
// - Background edges MUST remain visible.
// - Signature style elements (e.g. flames, smoke, sparks) are allowed ONLY if present in the style reference.
// - Do NOT crop or expand the background.
...
// - No text, no logos, no watermarks
// - No hands, no people
// - No utensils (no spoons, forks, knives)

// FAILURE AVOIDANCE (MUST FOLLOW):
// - If the style is not clearly visible on the dish, increase style strength on the dish
//   (lighting + color grading) until it matches the reference style.
// - Any change in framing, crop, scale, or camera distance is considered a FAILURE.
...
//     core,
//     FRAMING_BASE,
//     formatFramingHint(opts.formatId),
//     detailsBlock,
//   ]
//     .filter(Boolean)
//     .join("\n\n");
