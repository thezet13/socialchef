...
//       const srcPath = path.join(uploadsDirLocal, srcName);
//       if (!fs.existsSync(srcPath)) return res.status(404).json({ error: "Preview file not found" });

//       // copy to a new "base" file (so later cleanup of preview won't break base)
//       const finalId = `base_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
//       const dstName = `${finalId}.png`;
//       const dstPath = path.join(uploadsDirLocal, dstName);

//       fs.copyFileSync(srcPath, dstPath);

//       const newBaseImageUrl = `/uploads/images/${dstName}`;
...
// ВАЖНО: uploadsImagesDir импортируй из uploadsPaths
// import { uploadsImagesDir } from "@/lib/uploadsPaths";

aiRouter.post("/pro-images/:id/render",
  requireAuth,
  withTenant,
  async (req, res) => {
    try {
      ...
      if (saveToH) {
        ...
        await ensureAssetForUploadsUrl({
          tenantId,
          uploadsUrl: finalImageUrl,
          kind: "GENERATED_RENDER",
        });

        // ✅ cap 100 exports per tenant (delete oldest)
        ...
        for (const row of extra) {
          ...
          // optional: mark Asset deleted (если ты хочешь консистентность)
          try {
            await prisma.asset.updateMany({
              where: { tenantId, storagePath: row.imageUrl },
              data: { status: "DELETED" },
            });
          } catch {
            // ignore
...
// aiRouter.post("/pro-images/:id/bake-gpt15",
//   requireAuth, withTenant, async (req, res) => {
//     try {
//       if (!req.auth) return res.status(401).json({ error: "Unauthorized" });
//       const { userId, tenantId } = req.auth;
...
//       // ✅ NEW: bake uses ALL visible elements
//       const all = overlayVisible;

//       // optional: check there is at least something to bake
//       const hasAny =
...
//       if (!hasAny) {
//         return res.status(400).json({ error: "No overlay elements to bake" });
//       }
...
//       const targetW = body.outputWidth;
//       const targetH = body.outputHeight;

//       const aiSize = resolveAiEditSize(targetW, targetH);
//       const aiW = aiSize.w;
//       const aiH = aiSize.h;

//       // 1) BASE image (image[0])
//       const basePng = await loadUploadsAnyAsPng(design.baseImageUrl);
//       const baseFile = fileFromPng(basePng, "base.png");

//       // 2) LAYOUT GUIDE image (image[1]) — строим по ВСЕМ элементам
//       const { guidePng, guideItems } = await renderLayoutGuidePng({
//         baseW: body.baseWidth,
//         baseH: body.baseHeight,
//         outW: aiW,
//         outH: aiH,
//         baked: all, // (параметр можно потом переименовать, но так работает сразу)
//       });
//       const guideFile = fileFromPng(guidePng, "layout_guide.png");

//       // 3) BRAND STYLE reference image (image[2])
//       const styleRefPng = await loadUploadsAnyAsPng(body.styleRefImageUrl);
//       const styleFile = fileFromPng(styleRefPng, "brand_style.png");

//       // 4) Extra inputs for pics (optional)
//       const allPics = (all.pics ?? []).filter((p) => p?.visible !== false);

//       if (allPics.length > 6) {
//         return res.status(400).json({ error: "Too many pics. Max 6." });
//       }
...
//       const prompt = buildBakePrompt({ guideItems, behavior: body.behavior });
...
//       // resize/crop to exact targetW x targetH
//       const outCanvas = createCanvas(targetW, targetH);
//       const octx = outCanvas.getContext("2d");
//       const aiImg = await loadImage(aiOutPng);

//       // cover -> keeps full frame without padding, matches your export logic
//       drawImageCover(octx, aiImg as any, targetW, targetH, { scale: 1, offsetX: 0, offsetY: 0 });

//       const finalPng = await outCanvas.encode("png");

//       // save baked base
//       const { url: bakedBaseImageUrl } = savePngToUploads({
//         uploadsImagesDir: uploadsImagesDir,
//         prefix: "baked",
//         png: finalPng,
//       });

//       // update design: baseImageUrl becomes baked, overlay becomes ONLY front
//       await prisma.proDesign.update({
//         where: { id: design.id },
//         data: {
//           baseImageUrl: bakedBaseImageUrl,
...
//         proDesignId: design.id,
//         bakedBaseImageUrl,
//         // overlay после bake пустой (всё запекли)
//         overlayFront: { texts: [], pics: [], rects: [] },
//         aiSize,
//         outputWidth: targetW,
//         outputHeight: targetH,
//         bakedCounts: {
//           texts: all.texts?.length ?? 0,
//           rects: all.rects?.length ?? 0,
//           pics: all.pics?.length ?? 0,
//         },
//         creditsBalance: creditsBalanceAfter,
//       });
//     } catch (err) {
...
//       console.error("[POST /ai/pro-images/:id/bake-gpt15]", err);
//       return res.status(500).json({ error: "Internal server error" });
//     }
//   }
// );
